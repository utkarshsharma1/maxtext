...
class Decoder(nn.Module):
  ...
  def setup(self):
    ...
    self.decoder_layer = maxtext_utils.get_decoder_layers(self.config)
  ...
  def _apply_embedding(
      self, decoder_input_tokens, decoder_positions, deterministic, image_embeddings=None, bidirectional_mask=None
  ):
  ...
  @nn.compact
  def _apply_output_head(self, hidden_state, deterministic, model_mode):
    """Applies final normalization and projects hidden states to logits."""

    cfg = self.config

    hidden_state = self.get_norm_layer(num_features=hidden_state.shape[-1])(
        dtype=cfg.dtype,
        weight_dtype=cfg.weight_dtype,
        name="decoder_norm",
        epsilon=cfg.normalization_layer_epsilon,
    ...
    hidden_state = nn.Dropout(rate=cfg.dropout_rate, broadcast_dims=(-2,))(hidden_state, deterministic=deterministic)

    # [batch, length, emb_dim] -> [batch, length, vocab_size]
    if cfg.logits_via_embedding:
      # Use the transpose of embedding matrix for logit transform.
      logits = self.shared_embedding.attend(hidden_state)
      if self.config.normalize_embedding_logits:
        # Correctly normalize pre-softmax logits for this shared case.
        logits = logits / jnp.sqrt(hidden_state.shape[-1])
      if cfg.final_logits_soft_cap:
        logits = logits / cfg.final_logits_soft_cap
        logits = jnp.tanh(logits) * cfg.final_logits_soft_cap
    else:
      logits = linears.dense_general(
          inputs_shape=hidden_state.shape,
  ...
  def __call__(
    ...
    # [batch, length] -> [batch, length, emb_dim]
    y = self._apply_embedding(decoder_input_tokens, decoder_positions, deterministic, image_embeddings, bidirectional_mask)
