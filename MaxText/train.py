...
def loss_fn(model, config, data, dropout_rng, params, is_train=True):
  ...
  mtp_loss = 0.0
  if config.mtp_num_layers > 0 and is_train:
    mtp_loss = maxtext_utils.calculate_mtp_loss(intermediate_outputs, config)
    loss += mtp_loss
...
def eval_step(model, config, state, data, dropout_rng):
  ...
  mtp_acceptance_rate = maxtext_utils.calculate_mtp_acceptance_rate(aux["intermediate_outputs"], config)
